...
const Header = function Header({className}) {
  ...
  const handleClickConnect = () => {
    setTimeout(() => {
      if (typeof negotiator !== "undefined" || negotiator !== {}) {
  ...
  const isDemoRoute = [
    "/demo/fashion",
    "/demo/music-and-events",
    "/demo/automotive"
  ].find((route) => route === router.pathname);
      ...
      <div className={clsx(styles["a-header_bar"])}>
        ...
        <div className={styles["a-header_nav-container"]} ref={menuRef}>
          <Nav className={clsx(styles["a-header_nav"])}>
            <Nav.Item
              className={clsx(styles["a-header_nav-item"], {
                ["-is-active"]: router.pathname === "/"
              })}
            >
              <Link href="/" className="-style-headline">
                Get Tokens
              </Link>
            </Nav.Item>
            <Nav.Item
              className={clsx(styles["a-header_nav-item"], {
                ["-is-active"]: isDemoRoute
              })}
            >
              <Link
          ...
          >
            <div
              key="demo-menu"
              className={clsx(styles["c-popover"], {"-hide": !isMenuOpen})}
            >
              <p className="-f-caps f9 -mt2">Examples</p>
              <Link
                href="/demo/fashion"
                className="-style-headline -va-center -my1"
              >
                <Icon type="clothing" className="-color-primary -mr1" /> Fashion
              </Link>
              <Link
                href="/demo/music-and-events"
                className="-style-headline -va-center -my1"
              >
                <Icon type="music" className="-color-primary -mr1" /> Music &
                Event
              </Link>
              <Link
