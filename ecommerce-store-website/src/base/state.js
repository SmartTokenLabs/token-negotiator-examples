...
export const useStore = create(
  subscribeWithSelector((set) => ({
    ...
    api: {
      ...
      setContextView: (contextView, contextProps) =>
        set((state) => {
      ...
      setIsNegotiatorReady: (isNegotiatorReady) =>
        set(() => ({isNegotiatorReady})),
      setSelectedTokens: (selectedTokens) => {
        set(() => ({selectedTokens: selectedTokens}));
      },
      setTokenNegotiatorInstance: (tokenNegotiatorInstance) => {
        set(() => ({tokenNegotiatorInstance}));
      }
    }
  }))
);
