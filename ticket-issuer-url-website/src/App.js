  ...
  useEffect(() => {
    ...
    window.negotiator.on("tokens", (issuerTokens) => {
      ...
      tokenIssuers.forEach((issuer) => {
        // single issuer
        if (issuerTokens[issuer.collectionID].tokens.length > 0) {
          tokens.push(...issuerTokens[issuer.collectionID].tokens);
        }
        // multiple issuers
        if (issuerTokens[issuer.collectionID].tokens[[issuer.collectionID]]) {
          tokens.push(
            ...issuerTokens[issuer.collectionID].tokens[[issuer.collectionID]]
          );
        }
      });
      setTokens(tokens);
