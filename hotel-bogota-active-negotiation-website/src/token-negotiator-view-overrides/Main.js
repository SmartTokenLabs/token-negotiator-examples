...
import { TokenList } from './partials/token-list'
import { IconView } from './partials/icon-view'
...
export class Main extends AbstractView {
  ...
  init() {
    ...
    this.client.registerUiUpdateCallback('ISSUERS_LOADING', () => {
      this.issuersLoading()
    })

    this.client.registerUiUpdateCallback('ISSUERS_LOADED', () => {
    ...
    this.client.on('disconnected-wallet', () => {
  ...
  renderContent() {
    ...
    if (!this.issuerListContainer) return;
  ...
  populateIssuers() {
    ...
    this.issuerListContainer.addEventListener('click', (e) => {
  ...
  issuerConnectMarkup(title, image, issuer, tokens, data) {
    let buttonText = ''
  ...
  issuerLoadingComplete(issuer, tokens) {
    if (!tokens?.length) {
  ...
  issuerDidntConnect(issuer, showNotification = false) {
    const collectLoadAttempts = this.client.getTokenStore().getCollectionLoadAttempts(issuer);
    let issuerButtonText = this.params.options?.loadAction ?? "Load Collection";
    if (collectLoadAttempts >= 1) issuerButtonText = this.params.options?.repeatAction ?? 'Retry';
  ...
  issuerConnected(issuer, tokens, showTokens = true) {
    const connectBtn = this.issuerListContainer.querySelector(`[data-issuer="${issuer}"] .connect-btn-tn`)
