...
var TokenList = (function (_super) {
    ...
    TokenList.prototype.createTokenMarkup = function (config) {
        var tokenIssuerKey = config.tokenIssuerKey, title = config.title, data = config.data, index = config.index, image = config.image, toggleState = config.toggleState;
        var indToStr = index.toString();
        var tokenId = index > 15 ?
            (indToStr.substring(0, 5) + "..." + indToStr.substring(indToStr.length - 5, indToStr.length))
            : index;
